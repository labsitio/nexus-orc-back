...
| Integração externa disparada pela decisão | PARCIAL — cobertura de `RegistrarDecisaoHumanaWorkflow` confirmada; falta em `ConsolidarEDecidirWorkflow`/`RevisarDecisaoWorkflowComIA` | #253 (T047, teste), #255 (T049, ADR-003, estender os outros 2 casos de uso) | — |
...
| **P1** | #252, #253, #254, #255, #256 (005, reenvio + integração externa restante), #229, #238, #251, #664 | Critério (a): sem reenvio ao fornecedor e sem estender a integração externa aos outros 2 casos de uso, o fluxo ponta a ponta continua incompleto mesmo com #246/#248 fechadas. #664 é pré-requisito de qualquer caso de uso que chame o Agente Orquestrador via Bedrock real. |
