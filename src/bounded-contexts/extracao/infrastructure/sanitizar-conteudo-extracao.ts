 ...
 * insumo do prompt do Agente Extrator (mitigação de prompt injection — ver
 * "Segurança" em plan.md). O texto de um documento de fornecedor é SEMPRE
 * entrada não confiável: nunca cruza para o Domain, nunca é interpretado
 * como instrução, e nunca chega ao Bedrock sem passar por aqui.
 *
 * Réplica mecânica de `ingestao-identificacao/infrastructure/sanitizar-conteudo-documento.ts`
 * (spec-001) — instância própria deste BC (ADR-002, Princípio III), mesma
 * lógica de sanitização, zero decisão de design nova.
 *
 * Consumida por `MarkItDownConversaoExtracaoACL` (Infrastructure) antes de
 * devolver o texto convertido.
 */
 ...
 * string vazia, nunca interrompe o pipeline de extração.
 *
 ...
 */
export function sanitizarConteudoExtracao(textoBruto: string): string {
  if (!textoBruto) {
    return '';
  }
