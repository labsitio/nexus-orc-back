...
- [x] T019 [P] [US1] Contract test `GET /v1/orcamentos/{orcamentoId}/extracao/status` em `tests/bounded-contexts/extracao/contract/`. Schema Zod (`status.schema.ts`) criado junto (T024 ainda não existe) — espelha o shape real de `paraPayload()` do domínio, divergente do exemplo `montante`/`Dinheiro` provisório de `docs/openapi.yaml` (domínio usa `valorCentavos` inteiro). #84
