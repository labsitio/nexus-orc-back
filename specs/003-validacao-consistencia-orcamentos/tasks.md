...
- [x] T045 [US3] IAM: role dedicada `ValidarOrcamentoLambdaRole` (T028) estendida com `bedrock:InvokeModel` restrito ao ARN do modelo de categorização aprovado. Implementado em `validar-orcamento-lambda-role-stack.ts` (issue #155) — `CfnParameter ModeloCategorizacaoAprovadoArn` injetado no deploy, `PolicyStatement` com `Resource: [modeloCategorizacaoAprovadoArn.valueAsString]`, nunca wildcard; mesmo padrão de `IndexadorLambdaRoleStack`. Verificado por síntese CDK isolada (`Template.fromStack`) — LocalStack não aplica IAM, nenhuma prova de ambiente real.
