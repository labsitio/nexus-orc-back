...
- `MarkItDownConversaoACL` — invoca MarkItDown (processo isolado/Lambda Layer, CPU-bound, timeout e memória dimensionados) para gerar texto leve do documento; sanitiza o texto antes de compor o prompt do Classificador (ver Segurança). **Nota de execução local (2026-08-03, parecer do `arquiteto-back`)**: existe uma única implementação desta porta, a que invoca o Lambda dedicado via `lambda:Invoke` — em desenvolvimento local roda-se **o mesmo Lambda** no LocalStack Lambda (community suporta Lambda; Bedrock é Pro e fica fora), mudando apenas o endpoint do `LambdaClient`. Adapter alternativo por ambiente (subprocess/container local selecionado por variável de ambiente) foi avaliado e **rejeitado**: enquanto o Lambda de produção não existir, o adapter local seria a única implementação exercitada de ponta a ponta e todo aprendizado de edge case real de PDF/XLSX (encoding, timeout, memória, erro do MarkItDown) ficaria fora do artefato de produção. Tasks T066 (Lambda de produção) e T067 (execução local via LocalStack) fecham este gap — o ACL consumidor já estava mergeado (T030) sem que nenhuma task entregasse o Lambda que ele invoca.
