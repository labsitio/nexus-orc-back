...
Abordagem técnica: novo Bounded Context **Orquestração**, com agregado raiz próprio (`DecisaoWorkflow`) — nunca reaproveita os agregados de Ingestão/Extração/Validação. Comunicação de entrada exclusivamente via assinatura de eventos (Customer/Supplier, Orquestração é customer de Ingestão, Extração e Validação simultaneamente — não apenas de Validação; ver ADR-001 sobre como o contexto consolidado é construído sem chamada síncrona cross-BC). Governança de baixa confiança reaproveita o mesmo padrão de duas linhas já vinculante na constituição (Princípio IV) e replicado nas specs 001/002: Agente Orquestrador → (baixa confiança) Agente Revisor de IA → (baixa confiança) fila de escalonamento assíncrona do comprador (ADR-002, mesmo racional do ADR-003 da spec 002 — componente próprio deste BC, nunca físico compartilhado). Persistência Aurora Serverless v2 + Drizzle (ADR-001 da spec 001, herdado).
...
**Target Platform**: AWS Lambda atrás de API Gateway (endpoint de decisão humana e de status); consumidores SQS para os casos de uso assíncronos (3 filas de entrada de contexto + 1 fila de revisão + 1 fila de escalonamento); EventBridge custom bus `nexo-dominio-bus` (mesmo bus das specs 001–003).
...
**Performance Goals**: p95 ≤ 5 minutos entre "orçamento validado disponível" e "decisão de workflow publicada" (para decisões resolvidas por Orquestrador ou Agente Revisor) — meta definida na própria spec (seção Métricas), não medida ainda (produto novo).

**Constraints**: consolidação de contexto depende da ordem causal real do pipeline (Classificação → Extração → Validação sempre ocorrem antes da decisão de workflow para o mesmo `orcamentoId`), mas a entrega dos eventos via EventBridge/SQS não garante ordem de chegada ao consumidor deste BC — o caso de uso de decisão MUST tolerar chegada fora de ordem sem decidir com contexto incompleto (ver ADR-001, "contexto incompleto nunca é decidido, é reprocessado"); cold start do `AgenteOrquestradorGateway`/`AgenteRevisorWorkflowGateway` (chamada síncrona Bedrock) é a mesma consideração de design das specs 001–003; esta é a decisão de maior risco financeiro da cadeia — nenhuma otimização de custo/latência MUST comprometer a garantia de "nunca aprovar sem confiança suficiente reportada com base auditável" (ver Segurança).

**Scale/Scope**: 1 Bounded Context (Orquestração), 1 agregado raiz, 2 agentes de IA (Orquestrador + Agente Revisor de Workflow — este último é "agente adicional explícito" já autorizado pela constituição, análogo ao Revisor das specs 001/002), 1 fila de escalonamento humana própria deste BC, 3 eventos de entrada de contexto (Ingestão, Extração, Validação) + 1 evento de decisão consolidada.
...
|---|---|---|
| I. Rastreabilidade ponta a ponta | Tabela `decisoes_workflow_historico` append-only grava agente decisor (Orquestrador/Revisor/Humano), critério, nível de confiança e timestamp de cada tentativa; reconstruível por `orcamentoId` | PASS |
...
| IV. Exceção nunca é silenciosa | Cadeia Orquestrador → Agente Revisor de Workflow → fila de escalonamento assíncrona implementa Princípio IV(b) explicitamente; nenhum agente pode reportar confiança artificial (VO `NivelConfianca` reaproveitado, faixa 0–100, nunca number solto; decisão `APROVAR` MUST carregar `criterio` textual não vazio, tornando a base da confiança auditável — ver Segurança); fila nunca autoaprova por tempo/volume/exaustão (invariante estrutural do agregado, não apenas processo); histórico nunca sobrescrito | PASS |
| V. IA generativa como motor de entendimento | Orquestrador e Revisor de Workflow são 100% Bedrock; NUNCA decidem conteúdo de fornecedor/formato/extração/validação (já decidido pelos agentes anteriores) — atuam estritamente sobre o resultado consolidado; nenhuma regra fixa por fornecedor | PASS |
...
| Additional Constraint — 5 agentes, papéis fixos | Agente Orquestrador mapeado ao papel fixo "Orquestrador (workflow/roteamento)"; Agente Revisor de Workflow é agente adicional explícito, mesmo padrão já autorizado pela constituição para tratamento de exceção (Princípio IV), replicado das specs 001/002 | PASS |
...
[BC: Validação] --(evento)--> OrcamentoValidado/ComRessalva            ---+                                                  |
                                                                                                        [Agente Orquestrador (Bedrock)]
                                                                                                                              |
                    (confiança suficiente) -----------------------------------------+------(<confiança) DecisaoWorkflowBaixaConfiancaDetectada
                                            |                    |                  |                              |
                                  OrcamentoAprovadoParaProcessamento    OrcamentoEncaminhadoParaComprador   OrcamentoReenvioSolicitado
                                                                                                                              v
                                                                                                     [Agente Revisor de Workflow (Bedrock)]
                                                                                                                              |
                                            (confiança suficiente, mesmos 3 desfechos possíveis)         (<confiança) DecisaoWorkflowEscalonadaParaComprador
                                                                                                                              |
...
- **Atributos**:
  ...
  - `status` (VO `StatusDecisaoWorkflow`: `AGUARDANDO_CONTEXTO` | `CONTEXTO_CONSOLIDADO` | `DECIDIDO` | `EM_REVISAO_AUTOMATICA` | `PENDENTE_REVISAO_HUMANA`).
  - `decisaoAtual` (VO `DecisaoRoteamento`, opcional até haver decisão com confiança suficiente ou decisão humana: `{ acao: 'APROVAR' | 'ENCAMINHAR_COMPRADOR' | 'SOLICITAR_REENVIO', nivelConfianca: NivelConfianca | null (null apenas quando `agenteOrigem === 'HUMANO'`), criterio (texto não vazio, obrigatório), agenteOrigem: 'ORQUESTRADOR' | 'REVISOR' | 'HUMANO', requerIntegracaoExterna: boolean, motivoDadoAusente (obrigatório e não vazio quando `acao === 'SOLICITAR_REENVIO'`, referenciando uma inconsistência/pendência concreta do `contextoValidacao`/`contextoExtracao`) }`).
...
- **Invariantes** (aplicadas nos métodos do agregado, nunca na Application):
  ...
  - `registrarTentativaOrquestrador(resultado)`: só pode ser chamado a partir de `CONTEXTO_CONSOLIDADO`. Se `resultado.nivelConfianca < LIMIAR_CONFIANCA (80, parâmetro operacional — ver "Fora de escopo" do spec.md)`, transita para `EM_REVISAO_AUTOMATICA`, nunca decide. Se confiança suficiente, aplica as regras de negócio abaixo e transita para `DECIDIDO`:
  ...
  - `registrarTentativaRevisor(resultado)`: mesma regra do Revisor das specs 001/002 — só uma tentativa; se confiança < limiar, transita para `PENDENTE_REVISAO_HUMANA`, nunca reabre uma segunda tentativa automática; se confiança suficiente, mesmas regras de negócio de `registrarTentativaOrquestrador` (aprovação exige validação bem-sucedida, reenvio exige fundamento) se aplicam igualmente ao Revisor — nenhum agente automático é exceção a essas regras.
...
- `DecisaoRoteamento` — `{ acao, nivelConfianca, criterio, agenteOrigem, requerIntegracaoExterna, motivoDadoAusente }`; construtor aplica as invariantes de negócio descritas acima (é o VO mais crítico desta spec — nenhuma instância inválida é representável).
...
1. `OrcamentoAprovadoParaProcessamento` — publicado quando `DecisaoRoteamento.acao === 'APROVAR'` é registrada (Orquestrador, Revisor, ou decisão humana — campo `agenteOrigem` distingue). Consumido pelas etapas de negócio subsequentes (fora de escopo) e por Acompanhamento.
...
5. `DecisaoWorkflowBaixaConfiancaDetectada` — publicado internamente pelo caso de uso de decisão quando o Orquestrador não atinge confiança suficiente; único gatilho para acionar o Agente Revisor de Workflow (nunca chamada direta). Evento interno, não é contrato externo estável (mesmo padrão de `OrcamentoBaixaConfiancaDetectada` da spec 001).
6. `DecisaoWorkflowEscalonadaParaComprador` — publicado quando o Agente Revisor de Workflow também não atinge confiança suficiente. Consumido pelo Acompanhamento/consumidor externo para exibir "pendente de decisão de workflow".

Nota: os quatro primeiros eventos são os contratos externos estáveis desta spec — `DecisaoWorkflowBaixaConfiancaDetectada` é detalhe de implementação interno, mesmo padrão já estabelecido pelas specs 001/002 para os equivalentes de baixa confiança.
...
- `ConsolidarEDecidirWorkflow(orcamentoId, payload)` — consumidor de `OrcamentoValidado`/`OrcamentoValidadoComRessalva` (via SQS), o gatilho real da decisão (último evento da cadeia causal). Traduz via `OrcamentoValidadoEventACL`, aplica `registrarContextoValidacao`, tenta `consolidarContexto()`:
  ...
  - Se consolidado, invoca `AgenteOrquestradorGateway`, aplica `DecisaoWorkflow.registrarTentativaOrquestrador`, persiste, publica o evento de desfecho correspondente (+ `IntegracaoExternaSolicitada` se aplicável) ou `DecisaoWorkflowBaixaConfiancaDetectada`.
- `RevisarDecisaoWorkflowComIA(orcamentoId)` — consumidor de `DecisaoWorkflowBaixaConfiancaDetectada`. Invoca `AgenteRevisorWorkflowGateway` com contexto adicional (histórico de decisões anteriores do mesmo fornecedor, consultado via `buscarDecisoesAnterioresPorFornecedor` no próprio repositório deste BC — nunca uma chamada cross-BC), aplica `registrarTentativaRevisor`, persiste, publica o evento de desfecho correspondente ou `DecisaoWorkflowEscalonadaParaComprador`.
...
- `DrizzleDecisaoWorkflowRepository` — traduz linha↔agregado sobre Aurora Serverless v2 Postgres; tabelas `decisoes_workflow` (estado atual, `contexto_classificacao`/`contexto_extracao`/`contexto_validacao`/`decisao_atual` em colunas JSONB, mesmo racional YAGNI do ADR-004 da spec 002) e `decisoes_workflow_historico` (append-only, nunca UPDATE/DELETE, apenas INSERT); expõe `buscarDecisoesAnterioresPorFornecedor(fornecedorIdentificado)` para o contexto adicional do Revisor.
- Filas SQS por consumidor: `contexto-classificacao-queue`, `contexto-extracao-queue`, `decisao-workflow-queue` (gatilho de decisão), `revisor-workflow-queue`, cada uma com DLQ própria + alarme CloudWatch em mensagem na DLQ — a DLQ de `decisao-workflow-queue` é o mecanismo operacional que torna visível um contexto que nunca se consolida (Princípio IV, ver ADR-001).
- IAM: uma role por Lambda (`RegistrarContextoClassificacaoLambdaRole`, `RegistrarContextoExtracaoLambdaRole`, `ConsolidarEDecidirWorkflowLambdaRole`, `RevisarDecisaoWorkflowComIALambdaRole`, `RegistrarDecisaoHumanaWorkflowLambdaRole`, `ConsultaStatusDecisaoWorkflowLambdaRole`), least privilege — ex.: `ConsolidarEDecidirWorkflowLambdaRole` tem `bedrock:InvokeModel` restrito ao ARN do modelo aprovado, sem qualquer permissão sobre `nexo-orcamentos-raw` ou sobre as tabelas de outros BCs.
...
- Consumidor SQS (`revisor-workflow-queue`) acionado por regra EventBridge roteando `detail-type: DecisaoWorkflowBaixaConfiancaDetectada` (interno) → fila deste BC.
...
└── bounded-contexts/
    └── orquestracao/
        ...
        │   ├── events/ (orcamento-aprovado-para-processamento, orcamento-encaminhado-para-comprador, orcamento-reenvio-solicitado, integracao-externa-solicitada, decisao-workflow-baixa-confianca-detectada, decisao-workflow-escalonada-para-comprador)
        │   ├── repositories/ (decisao-workflow.repository.ts — interface)
        │   └── gateways/ (agente-orquestrador.gateway.ts, agente-revisor-workflow.gateway.ts, orcamento-classificado-event.acl.ts, orcamento-extraido-event.acl.ts, orcamento-validado-event.acl.ts — interfaces)
        ├── application/
        │   └── use-cases/ (registrar-contexto-classificacao, registrar-contexto-extracao, consolidar-e-decidir-workflow, revisar-decisao-workflow-com-ia, registrar-decisao-humana-workflow, consultar-status-decisao-workflow)
        ├── infrastructure/
        │   ├── persistence/ (drizzle-decisao-workflow.repository.ts, schema/)
        │   ├── aws/ (eventbridge.publisher.ts)
        │   └── bedrock/ (bedrock-orquestrador.gateway.ts, bedrock-revisor-workflow.gateway.ts, acl/)
        └── interface/
...
**Structure Decision**: mesma convenção das specs 001–003 — novo subdiretório `src/bounded-contexts/orquestracao/` isolado, sem import direto de código de `ingestao-identificacao/`, `extracao/` ou `validacao/`; toda comunicação de entrada via evento, consumida por SQS (quatro filas distintas, refletindo os quatro pontos de entrada de evento desta spec).
...
### ADR-002 — Governança de baixa confiança da Orquestração é padrão replicado (Orquestrador → Revisor de Workflow → escalonamento), componente próprio deste BC

**Contexto**: mesma decisão de desenho já enfrentada pela spec 002 (ADR-003 daquela spec) — a spec.md desta feature explicitamente reaproveita "o mesmo padrão de duas linhas já vinculante na constituição e já usado na spec 001", sem exigir componente físico compartilhado.

**Problema**: replicar componente próprio (Agente Revisor de Workflow + fila de escalonamento) vs. reutilizar algum componente físico das specs 001/002/003.

**Alternativas consideradas**: (a) componente físico compartilhado entre BCs; (b) padrão replicado, componentes próprios por BC (mesma decisão já tomada em 002/ADR-003 e implicitamente em 003/ADR-001, que optou por *não* replicar o Revisor por razão específica àquela spec, mas manteve o princípio de "decisão por BC, nunca componente físico compartilhado").

**Vantagens (opção b, escolhida)**: mesma linha de raciocínio do ADR-003 da spec 002 — preserva autonomia de deploy, preserva "cada BC decide sua própria pergunta de negócio" (aqui, "o roteamento é confiável?", distinta de "a extração é confiável?" ou "a validação passou?"), preserva Princípio II.

**Desvantagens**: mesma replicação de estrutura (fila + Lambda + gateway Bedrock) já aceita como trade-off nas specs 001/002.

**Decisão**: Orquestração implementa `AgenteRevisorWorkflowGateway` e sua própria fila de escalonamento assíncrona (`revisor-workflow-queue` para acionar o Revisor + estado `PENDENTE_REVISAO_HUMANA` no próprio agregado `DecisaoWorkflow` para o escalonamento final ao comprador), seguindo o mesmo padrão de estados/transições das specs 001/002, sem nenhuma chamada cross-BC.

**Trade-offs**: idêntico ao ADR-003 da spec 002 — menos reuso físico em troca de zero acoplamento entre BCs.

**Impactos futuros**: nenhum além do já registrado pelo ADR-003 da spec 002 — este ADR apenas confirma a aplicação consistente do mesmo padrão à quarta spec do pipeline que precisa dele.
...
(b) O próprio agente decisor (Orquestrador/Revisor de Workflow) reporta `requerIntegracaoExterna` como parte de sua saída estruturada, junto de `acao`/`nivelConfianca`/`criterio`, e o Domain apenas valida a forma (booleano presente), sem impor a regra de quando é verdade;
