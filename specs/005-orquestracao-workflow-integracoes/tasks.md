...
- [x] T009 [P] Domain: implementar VOs `ContextoClassificacao`, `ContextoExtracao`, `ContextoValidacao` (cópias imutáveis traduzidas dos payloads upstream, ver `plan.md` seção Domain).
...
- [x] T021 [P] [US1] Unit test `DecisaoWorkflow.registrarTentativaOrquestrador` com confiança suficiente e `contextoValidacao.resultado === 'VALIDADO'` → transita para `DECIDIDO`, publica o desfecho correspondente à `acao` reportada pelo agente. Coberto em `tests/bounded-contexts/orquestracao/domain/aggregates/decisao-workflow.aggregate.test.ts` — describe `registrarTentativaOrquestrador`: caso pré-existente ENCAMINHAR_COMPRADOR (linhas 198-212, `contextoValidacao.resultado === 'VALIDADO'`) + novo caso APROVAR com `resultado === 'VALIDADO'` (a cobertura pré-existente de APROVAR só usava `VALIDADO_COM_RESSALVA`) + novo caso happy-path de SOLICITAR_REENVIO (ação em que `contextoValidacao.resultado` não influencia o branch de `DecisaoRoteamento.criar`), cada um assertando `status === 'DECIDIDO'`, `decisaoAtual.acao` correspondente e histórico com o desfecho registrado.
- [x] T022 [P] [US1] Unit test da invariante "nunca aprovar sem validação bem-sucedida" (T010) especificamente no fluxo de `registrarTentativaOrquestrador` — tentar `acao: 'APROVAR'` com `contextoValidacao` ausente/reprovado lança `AprovacaoSemValidacaoError`. Coberto em `tests/bounded-contexts/orquestracao/domain/aggregates/decisao-workflow.aggregate.test.ts` — describe `registrarTentativaOrquestrador`, novo caso via `DecisaoWorkflow.reconstituir` com status `CONTEXTO_CONSOLIDADO` e `contextoValidacao` ausente (único caminho real, já que o VO `ContextoValidacao` só admite resultados aprováveis).
- [x] T023 [P] [US1] Contract test `GET /v1/orcamentos/{orcamentoId}/workflow/status` em `tests/bounded-contexts/orquestracao/contract/`. `tests/bounded-contexts/orquestracao/contract/status.controller.test.ts` já existia desde T030/#236 (7 casos: AGUARDANDO_CONTEXTO, DECIDIDO, PENDENTE_REVISAO_HUMANA, 404 inexistente, 400 malformado, 404 tenant divergente, 500 propagado) — issue #229 completou a cobertura de contrato com 2 casos que faltavam: 200 `CONTEXTO_CONSOLIDADO` (asserta a serialização completa de `contextoClassificacao`/`contextoExtracao`/`contextoValidacao`, nunca antes verificada) e 401 Problem Details do fallback defensivo do controller (`tenantContext` ausente).
...
- [x] T027 [US1] Application: caso de uso `RegistrarContextoExtracao` (mesmo padrão, consome `OrcamentoExtraido`/`ComPendenciaConfirmada`). Implementado em `src/bounded-contexts/orquestracao/application/use-cases/registrar-contexto-extracao.ts` (#233) — mesmo padrão de `RegistrarContextoClassificacao` (T026), traduzindo via `OrcamentoExtraidoEventACL` já entregue (T017/#650) e delegando ao agregado `DecisaoWorkflow.registrarContextoExtracao`.
- [x] T028 [US1] Application: caso de uso `ConsolidarEDecidirWorkflow` (consome `OrcamentoValidado`/`ComRessalva`, traduz via ACL, tenta `consolidarContexto()`; se sucesso, invoca `AgenteOrquestradorGateway`, aplica `registrarTentativaOrquestrador`, persiste, publica evento de desfecho) — caminho feliz de confiança suficiente. Implementado em `src/bounded-contexts/orquestracao/application/use-cases/consolidar-e-decidir-workflow.ts` (#234), cobrindo também o caminho de baixa confiança (T040) na mesma transição de Domain — ver nota em T040.
- [x] T029 [US1] Interface: handlers Lambda consumidores de `contexto-classificacao-queue`, `contexto-extracao-queue` e `decisao-workflow-queue`, invocando os 3 casos de uso acima. Implementado em `src/bounded-contexts/orquestracao/interface/events/{contexto-classificacao,contexto-extracao,decisao-workflow}-queue.handler.ts` (#235) — mesmo padrão de `busca-indexacao/interface/events/indexador-queue.handler.ts` (#190/PR #660): batch SQS, ACL chamada no handler para correlação de log (`orcamentoId`/`tenantId`) e novamente dentro do caso de uso, `batchItemFailures` item a item. Apenas os handlers (testáveis isoladamente); o entrypoint `export const handler` com wiring real de AWS SDK/composition root fica para a issue de deploy (#624), que agora está desbloqueada.
- [x] T030 [US1] Interface: controller `GET /v1/orcamentos/{orcamentoId}/workflow/status` (query, Zod schema de response, Problem Details para erro). Implementado em `src/bounded-contexts/orquestracao/interface/http/status.controller.ts` (#236) — mesmo padrão de `validacao/interface/http/status.controller.ts` (T026/#136): caso de uso de query `ConsultarStatusDecisaoWorkflow` (`application/use-cases/consultar-status-decisao-workflow.ts`), schema Zod (`status.schema.ts`), `route-opts.ts` para injeção de `preHandler` (T031, ainda não implementado). Autenticação Cognito real e wiring de composição raiz ficam para a issue de deploy, mesma nota já registrada em T029.
- [x] T031 [US1] Interface: autenticação Cognito (JWT) no endpoint de status, mesmo esquema das specs 001–003. Implementado em `src/bounded-contexts/orquestracao/interface/http/auth-cognito.middleware.ts` (#237) — mesmo padrão de `validacao/interface/http/auth-cognito.middleware.ts` (T027): `criarAutenticacaoCognito` gera `preHandlerHookHandler` injetável via `route-opts.ts` já preparado (T030). Wiring real de composição raiz (Lambda) fica para a issue de deploy, mesma nota já registrada em T029/T030.
...
- [x] T040 [US2] Application: completar `ConsolidarEDecidirWorkflow` (T028) para o caminho de baixa confiança — transitar o agregado para `PENDENTE_REVISAO_HUMANA` e publicar `DecisaoWorkflowEscalonadaParaComprador` diretamente quando o Orquestrador não atinge confiança suficiente. (#246) Já entregue junto de T028 (#234): a transição de estado é regra do agregado (`DecisaoWorkflow.registrarTentativaOrquestrador`, `decisao-workflow.aggregate.ts`), o caso de uso só lê `status` pós-transição e publica `DecisaoWorkflowEscalonadaParaComprador` (`tenantId` obrigatório, `schemaVersion: 2`) sem `if` de máquina de estado na application. Coberto por `tests/bounded-contexts/orquestracao/application/consolidar-e-decidir-workflow.test.ts` (caso "T040 — confiança insuficiente...") e `tests/bounded-contexts/orquestracao/domain/aggregates/decisao-workflow.aggregate.test.ts` (caso "confiança insuficiente transita direto para PENDENTE_REVISAO_HUMANA").
- [x] T042 [US2] Application: caso de uso `RegistrarDecisaoHumanaWorkflow` (valida status `PENDENTE_REVISAO_HUMANA`, aplica `registrarDecisaoHumana`, publica evento de desfecho com `agenteOrigem: 'HUMANO'`). Implementado em `src/bounded-contexts/orquestracao/application/use-cases/registrar-decisao-humana-workflow.ts` (#248) — mesmo padrão de `ConsolidarEDecidirWorkflow` (T028): busca o agregado, delega a `DecisaoWorkflow.registrarDecisaoHumana` (transição de estado é regra do agregado, sem `if` de máquina de estado na application), persiste e publica o evento de desfecho + `IntegracaoExternaSolicitada` quando `requerIntegracaoExterna`. Extraído `criar-evento-desfecho.ts` compartilhado com `ConsolidarEDecidirWorkflow` para não duplicar o mapeamento `acao` → evento.
- [x] T044 [US2] Interface: controller `POST /v1/orcamentos/{orcamentoId}/workflow/decisao-humana`, Zod schema (`acao`, `justificativa` obrigatória, `motivoDadoAusente` obrigatório quando `acao === 'SOLICITAR_REENVIO'`), papel "comprador responsável" via Cognito, Problem Details para 409. Implementado em `src/bounded-contexts/orquestracao/interface/http/decisao-humana.controller.ts` + `decisao-humana.schema.ts` (#250) — expõe `RegistrarDecisaoHumanaWorkflow` (T042/#248), guard `criarExigenciaPapel(['comprador-responsavel'])` (ADR-010) sempre concatenado ao final do array de `preHandler`, resposta 200 montada via `ConsultarStatusDecisaoWorkflow` (mesmo contrato de `GET .../workflow/status`). Absorve #688 (rota já nasce protegida). Testes em `tests/bounded-contexts/orquestracao/contract/decisao-humana.controller.test.ts` (11 casos: 200/400/404/409/403 + ordem preHandler).
...
- [x] T046 [P] [US3] Unit test `DecisaoRoteamento` (T010) — construtor rejeita `acao: 'SOLICITAR_REENVIO'` sem `motivoDadoAusente` não vazio; aceita quando `motivoDadoAusente` é uma string não vazia. Cobertura já entregue integralmente desde T010 (#497/#500) em `tests/bounded-contexts/orquestracao/domain/value-objects/decisao-roteamento.vo.test.ts` (`rejeita SOLICITAR_REENVIO sem motivoDadoAusente`, `rejeita SOLICITAR_REENVIO com motivoDadoAusente vazio/whitespace`, `aceita SOLICITAR_REENVIO com motivoDadoAusente concreto`). Nota: o VO valida apenas não-vacuidade de `motivoDadoAusente`, não faz cross-reference estrutural com `contextoValidacao`/`contextoExtracao` (o VO nem recebe `contextoExtracao` como input) — a "referência concreta" exigida pelo texto da spec é responsabilidade de quem monta o input (Application/Agente Orquestrador), não uma invariante do VO. Divergência entre o texto de T046/spec.md e o desenho real do VO registrada aqui; encaminhar ao arquiteto-back se uma invariante estrutural de cross-reference for de fato desejada.
...
- [x] T050 [US3] Domain: garantir que `ReenvioSemFundamentoError` inclui referência legível ao que faltou validar/preencher (nunca mensagem genérica) — mesma disciplina de `InconsistenciaDetectada.detalhe` da spec 003. (#256) `ReenvioSemFundamentoError` passou a receber `motivoDadoAusenteRecebido: string | undefined` e monta a mensagem descrevendo exatamente o que foi recebido — "não foi informado" quando `undefined`, ou o valor whitespace literal entre aspas quando vazio só de espaços — em vez do texto estático anterior (`decisao-roteamento.vo.ts`). Único ponto de construção do erro (`DecisaoRoteamento.criar`) atualizado para passar `input.motivoDadoAusente`. Coberto por asserções de mensagem (regex) em `decisao-roteamento.vo.test.ts`, mantendo as asserções de tipo (`toThrow(ReenvioSemFundamentoError)`) já existentes.
