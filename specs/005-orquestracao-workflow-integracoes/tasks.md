...
- [x] T021 [P] [US1] Unit test `DecisaoWorkflow.registrarTentativaOrquestrador` com confiança suficiente e `contextoValidacao.resultado === 'VALIDADO'` → transita para `DECIDIDO`, publica o desfecho correspondente à `acao` reportada pelo agente. Coberto em `tests/bounded-contexts/orquestracao/domain/aggregates/decisao-workflow.aggregate.test.ts` — describe `registrarTentativaOrquestrador`: caso pré-existente ENCAMINHAR_COMPRADOR (linhas 198-212) + novos casos APROVAR e SOLICITAR_REENVIO com `contextoValidacao.resultado === 'VALIDADO'`, cada um assertando `status === 'DECIDIDO'`, `decisaoAtual.acao` correspondente e histórico com o desfecho registrado.
