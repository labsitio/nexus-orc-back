...
## #746 (PR #749) — `OllamaInterpretadorConsultaGateway` + `selecionarAgenteInterpretador` (paridade ADR-009)

Fecha o gap de paridade ADR-009: `AgenteInterpretadorConsultaGateway` era a única porta de IA sem contraparte Ollama (classificador #617, extrator #619, orquestrador #621, embedding #620 já tinham). Mesmo padrão de #620/#736: gateway sobre `POST /api/chat` do Ollama, `format` com JSON Schema real (nunca `format: 'json'` livre), tradução do JSON bruto delegada à `BedrockInterpretacaoConsultaACL` (T033, já validada acima) — reaproveitada sem duplicação e agnóstica de qual modelo produziu o shape.

| Requisito / critério (issue #746, ADR-009) | Cenário | Teste | Resultado |
|---|---|---|---|
| Gateway invoca `POST /api/chat` com `format` = JSON Schema real (`enum` de `categoria` = `catalogoCategorias` da chamada) e devolve `CriterioBusca` traduzido pela ACL | caso feliz completo | `interpretar chama POST /api/chat com JSON Schema real em "format" (enum de categoria) e devolve CriterioBusca traduzido pela ACL` | PASS |
| Consulta do usuário isolada em bloco `<consulta_do_usuario>` na mensagem de usuário, nunca na instrução de sistema (mitigação de prompt injection, mesma disciplina de T037) | consulta contendo tentativa de injeção ("IGNORE AS REGRAS ANTERIORES...") | `isola a consulta do usuário em mensagem de usuário (nunca instrução de sistema)` | PASS |
| Falha HTTP (`resposta.ok === false`) é erro explícito, nunca silenciosa | status 500 | `lança erro se a requisição HTTP falhar` | PASS |
| Resposta sem `message.content` é erro explícito | `{ message: {} }` | `lança erro se a resposta não tiver message.content` (adicionado pelo QA — fechava único ramo não coberto do arquivo) | PASS |
| `message.content` sintaticamente inválido (não-JSON) é erro explícito, nunca parsing de texto livre por regex | `content: 'não é json'` | `lança erro se message.content não for JSON válido` | PASS |
| Shape do JSON retornado fora do esperado é rejeitado pelo type guard `ehInterpretacaoConsultaBruta`, nunca confia cegamente no LLM | `{ categoria: 'embalagens' }` (sem `textoLivreResidual`) | `lança erro se o JSON retornado não tiver o shape de interpretação esperado` | PASS |
| **Defesa em profundidade**: categoria fora do catálogo é rejeitada pela ACL mesmo que o modelo burle o `enum` do schema | `categoria: 'CATEGORIA_INVENTADA'` | `propaga rejeição da ACL quando "categoria" está fora do catálogo configurado` | PASS |
| `selecionarAgenteInterpretador('local')` constrói `OllamaInterpretadorConsultaGateway`; `('bedrock')` constrói `BedrockInterpretadorConsultaGateway` | ambos os valores válidos | 2 testes dedicados | PASS |
| `selecionarAgenteInterpretador` falha rápido se a config exigida pelo valor escolhido não foi fornecida (`config.bedrock`/`config.ollama` ausente) | 2 casos | 2 testes dedicados | PASS |
| `selecionarAgenteInterpretador` falha rápido se `NEXO_AGENTE_IA` ausente ou fora de `local`/`bedrock` | `undefined`, `'outro'` | `falha rápido se NEXO_AGENTE_IA estiver ausente ou com valor inválido` | PASS |
| `src/dev/local.ts` usa o seletor real (não mais o stub `interpretadorConsultaLocal`) e `catalogoCategorias` real, lido de `faixas_preco_categoria` no boot | leitura de código — `validacao.gatewayFaixaPreco.listarTodas()` já construído antes da leitura (linha 202 vs. 248), sem risco de uso antes da inicialização | inspeção de `src/dev/local.ts` (diff) | PASS (verificação estática — não há integração automatizada do runner local nesta suíte, mesma limitação já registrada para os demais wirings de `src/dev/local.ts`) |
| Nenhuma regressão | suíte completa | `npx vitest run --reporter=default` | PASS — 198 arquivos passando/19 pulados (217), 1290 testes passando/110 pulados (1400), 0 falhos. 3 arquivos de infra (`infra/lib/*-stack.test.ts`) falharam por timeout de hook (30s) na mesma rodada sob carga cheia da suíte; reexecutados isolados, passaram (23-24s cada) — flakiness pré-existente de CDK synth, não relacionada a este diff (nenhum arquivo de `infra/` alterado por #746) |

Cobertura isolada dos arquivos alterados (`--coverage.include`, relatório v8): `ollama-interpretador-consulta.gateway.ts` 100% statements/branches/functions/lines (20/20 stmts, 8/8 branches) após o teste adicionado pelo QA (baseline do dev-back-end: 95% stmts/lines, 87.5% branches — gap no guard `!conteudo`, nunca exercitado por nenhum dos 6 testes originais, todos com `message.content` truthy); `composition/busca-indexacao.ts` 90.62% statements/100% branches/62.5% functions — único gap são as linhas 117-127 (`repositorioNuncaUsado`, mesmo stub estruturalmente inalcançável já aceito nas entradas de #623/#620 acima; `selecionarAgenteInterpretador` em si está 100% coberta em todos os ramos). `npx tsc --noEmit` limpo; `npx eslint .` limpo. `pnpm audit` sem vulnerabilidades conhecidas (confirma o bump pontual `nanoid ^3.3.18` no mesmo PR, fix de segurança não relacionado à feature — GHSA-2v37-7h3g-55p8 atualizado para cobrir também 3.3.17).

`docs/poc-ollama-interpretador-consulta.md` documenta validação manual contra Ollama real (`llama3.1`): JSON estruturado devolvido com `format` real (não `'json'` livre), `categoria` respeitando o `enum` do catálogo, e um caso de ACL rejeitando explicitamente uma data inválida gerada pelo próprio modelo (limite conhecido de modelo pequeno sem noção de data corrente, documentado como fora de escopo). QA leu o documento e confirma consistência com os testes automatizados — nenhuma alegação de comportamento não coberto pela suíte.

Sem wiring de rota HTTP de produção nesta PR (`registrarRotaBuscaOrcamentos` só é chamado em `src/dev/local.ts`) — confirmado por grep, escopo correto de #746 (só gateway + seletor), consumo em produção fica para issue futura de composição HTTP deste BC.
