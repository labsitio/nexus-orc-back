...
- [ ] T030 [US1] Interface: handler Lambda consumidor SQS de `indexador-queue`, invocando `IndexarOrcamento`. **GATE (#190)**: MUST NOT ser implementada/mergeada antes de `specs/007-isolamento-multitenant-dados/tasks.md` T042 estar mergeada (ADR-008) — hoje o evento `OrcamentoValidado` recebido não carrega `tenantId`; inventar, inferir do payload ou consultar tabela de outro BC são todas soluções rejeitadas.
