...
- [ ] T012b **(ADR-005, retrofit)** Domain: adicionar atributo `tenantId: TenantId` (import de `src/shared-kernel/tenant/tenant-id.vo.ts`, spec 007) ao agregado `IndiceOrcamento`, obrigatório e imutável desde a criação; invariante `TenantIdImutavelError` em tentativa de sobrescrita, mesmo padrão de `OrigemValidacaoImutavelError`. Critério: unit test cobrindo criação sem `tenantId` (erro), sobrescrita pós-criação (erro).
...
- [ ] T013b **(ADR-005, retrofit)** Domain: subir os 2 Domain Events de T013 para `schemaVersion: 2`, payload incluindo `tenantId` obrigatório — mesma convenção de amendment já aplicada pela spec 007 aos eventos de 001 (ADR-005 daquela spec).
...
- [ ] T015b **(ADR-005, retrofit — BLOCKER apontado por backend-reviewer no PR #176)** Infrastructure: nova migração Drizzle Kit adicionando `tenant_id UUID NOT NULL` + `CREATE POLICY tenant_isolation ON indices_orcamento USING (tenant_id = current_setting('app.current_tenant_id')::uuid)` (e mesma política em `indices_orcamento_historico`) — mesmo padrão exato de `drizzle/0013_rls_orcamentos_tenant_isolation.sql` (spec 007). Pré-requisito de T016. Verificar junto ao checklist T009 da spec 007 (nenhuma role IAM com `BYPASSRLS`).
- [ ] T016 Infrastructure: `DrizzlePgvectorIndiceOrcamentoRepository` implementando `IndiceOrcamentoRepository` — inclui `upsert` idempotente por `orcamentoId`+`tenantId` (necessário para retry, T012) e método `buscarPorCriterioEVetor(tenantId, ...)` (filtro SQL determinístico AND `ORDER BY embedding <=> :vetor LIMIT :n`), traduzindo linha↔agregado, nunca vazando o tipo `vector` bruto para fora da Infra. **(ADR-005) MUST estender `DrizzleTenantScopedRepositoryBase` (`src/shared-kernel/tenant/`, spec 007/T008), usando `transacaoTenantScoped` em toda transação — depende de T015b.** Não considerar T016/PR #176 pronto sem esse retrofit.
...
- [ ] T019 Configurar logging estruturado (pino) + OpenTelemetry Node SDK para os handlers Lambda deste BC, correlação por `orcamentoId`/`tenantId` (mesma trilha ponta a ponta das specs 001–003 e 007).
...
- [ ] T024 [P] [US1] Contract test `GET /v1/orcamentos/{orcamentoId}/indexacao/status` em `tests/bounded-contexts/busca-indexacao/contract/`, incluindo caso Tenant A consultando `orcamentoId` de Tenant B → 404 (mesmo padrão T011 da spec 007).
