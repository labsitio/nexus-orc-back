...
- [x] T006 Coordenar com owner da spec 003 (Validação) o enriquecimento do payload de `OrcamentoValidado`/`OrcamentoValidadoComRessalva` para incluir `itens` e `condicoesComerciais` (ADR-003 do `plan.md`) — pré-requisito bloqueante; sem este enriquecimento, `OrcamentoValidadoEventACL` (T018) não tem dado de origem para montar `ConteudoIndexavel`. **Resolvido**: investigação confirmou que o payload real (`src/bounded-contexts/validacao/domain/events/orcamento-validado.event.ts`/`orcamento-validado-com-ressalva.event.ts`) ainda não trazia `itens`/`condicoesComerciais` (só `orcamentoId`/`ocorreuEm`, e `inconsistencias` no caso da ressalva) — enriquecimento implementado nesta mesma issue (#166): ambos os eventos passam a carregar `itens: ItemParaValidacaoPayload[]` e `condicoesComerciais: string` (mesmo shape de `DadosExtraidosParaValidacao`), `schemaVersion` sobe para `2` (amendment, mesmo padrão do retrofit ADR-005 já aplicado a `OrcamentoIndexado`/`FalhaIndexacaoDetectada`); `ValidarOrcamento` (spec 003) atualizado para publicar o payload enriquecido. Backward-compatible: `OrcamentoValidadoEventACL` (orquestração, spec 005) não lê os campos novos, continua funcionando sem alteração. **`tenantId` permanece pendente** — não está no escopo desta task (ver nota em T045/ADR-005): nenhum agregado do BC Validação (nem Ingestão/Extração upstream) carrega `tenantId` hoje; é uma segunda dependência de coordenação, maior e cross-spec (001→002→003), explicitamente registrada como risco remanescente separado pelo próprio ADR-005 do `plan.md` ("Impactos futuros"), não resolvida por este enriquecimento pontual. (#166 — coordenação fechada; implementação de código real é `specs/007-isolamento-multitenant-dados/tasks.md` T041, fundida com o retrofit de `tenantId` de ADR-008, não uma task separada aqui.)
...
- [x] T018 Infrastructure: `OrcamentoValidadoEventACL` traduzindo o payload (enriquecido, ver T006) dos eventos `OrcamentoValidado`/`OrcamentoValidadoComRessalva` para `ConteudoIndexavel` + `OrigemValidacao` — nunca importa tipos de domínio do BC Validação. Implementado em `src/bounded-contexts/busca-indexacao/infrastructure/orcamento-validado-event.acl.ts` (type guard estrutural sobre `payloadBruto: unknown`) contra a interface já congelada em T014 (`OrcamentoValidadoEventACLResultado`, sem `tenantId`) — payload real de `OrcamentoValidado`/`OrcamentoValidadoComRessalva` (spec 003, `schemaVersion: 2`) ainda não carrega `tenantId` (ver T045); `+ tenantId (ADR-005)` desta task permanece bloqueado por essa dependência cross-spec, não resolvido aqui.
...
- [x] T023 [P] [US1] Unit test `OrcamentoValidadoEventACL` (mock de payload enriquecido de `OrcamentoValidado`/`OrcamentoValidadoComRessalva`) → produz `ConteudoIndexavel` + `OrigemValidacao` corretos, preservando ambas as origens (ver ADR-004). Coberto por `tests/bounded-contexts/busca-indexacao/infrastructure/orcamento-validado-event.acl.test.ts` (15 casos: origem VALIDADO/VALIDADO_COM_RESSALVA, dedup de categorias, item sem descrição/categoria, todos os ramos do type guard estrutural, `OrcamentoId` inválido, `ConteudoIndexavel` vazio), já implementado no mesmo commit de T018 (#564). **`TenantId` não incluído** — `OrcamentoValidadoEventACLResultado` (interface congelada em T014) não carrega `tenantId`; payload upstream ainda não o propaga (dependência cross-spec bloqueante, ver T045/ADR-005). Descrição original desta task previa `tenantId` antes desse bloqueio ser confirmado em T018; nada a testar aqui até T045 ser resolvido.
...
- [x] T025 [P] [US1] Integration test: `OrcamentoValidado` publicado → `OrcamentoIndexado` publicado, embedding persistido em `indices_orcamento` com `tenant_id` correto, p95 medido em ambiente de teste local (LocalStack + Postgres/pgvector). Coberto por `tests/bounded-contexts/busca-indexacao/application/indexar-orcamento.integration.test.ts` (#185) — exercita `OrcamentoValidadoEventACL` + `IndexarOrcamento` (T029) + `DrizzlePgvectorIndiceOrcamentoRepository` (T016) reais contra Postgres/pgvector local; `AgenteEmbeddingGateway`/`EventPublisher` seguem fake (Bedrock/EventBridge têm suíte própria). `tenantId` fornecido diretamente ao caso de uso (contrato já existente de `IndexarOrcamento.executar`) — handler Lambda real (T030) permanece bloqueado por #190/ADR-008 (spec-007 T042 ainda não mergeada), nada implementado aqui antecipa esse handler.
...
- [x] T029 [US1] Application: caso de uso `IndexarOrcamento(tenantId, ...)` (consome `OrcamentoValidado`/`OrcamentoValidadoComRessalva`, traduz via ACL, invoca `AgenteEmbeddingGateway`, aplica `registrarTentativaIndexacao`, persiste via upsert idempotente, publica `OrcamentoIndexado` ou `FalhaIndexacaoDetectada` — ambos `schemaVersion: 2` com `tenantId`, ver T013b).
- [x] T030 [US1] Interface: handler Lambda consumidor SQS de `indexador-queue`, invocando `IndexarOrcamento`. **GATE (#190) resolvido**: `specs/007-isolamento-multitenant-dados/tasks.md` T042 (ADR-008) mergeada (#655) — `tenantId` obrigatório desde `schemaVersion: 2` do envelope de 003. Implementado em `src/bounded-contexts/busca-indexacao/interface/events/indexador-queue.handler.ts` (`criarIndexadorQueueHandler`) — usa `OrcamentoValidadoEventACL` (T014/T018) para extrair/validar `tenantId` e `detail-type` do envelope, nunca parseando à mão; batch item failures (`reportBatchItemFailures`) por mensagem, idempotência via `IndexarOrcamento.executar` (T029, já idempotente). Composição de produção (Lambda real, IAM) fica para #623, escopo separado (deploy).
- [x] T031 [US1] Interface: controller `GET /v1/orcamentos/{orcamentoId}/indexacao/status` (query, Zod schema de response, Problem Details para erro) + autenticação Cognito (JWT) atrás do `TenantContextMiddleware` compartilhado (spec 007) — `tenantId` do JWT comparado ao `tenantId` do agregado antes de retornar dado, 404 em divergência. Implementado via `ConsultarStatusIndexacao` (Application) + `registrarRotaStatusIndexacao` (Interface, #191) — composição raiz do handler Lambda (wiring de `criarAutenticacaoCognito`/`TenantContextMiddleware` real e `DrizzlePgvectorIndiceOrcamentoRepository`) fora do escopo desta task.
...
- [x] T034 [P] [US2] Unit test do caso de uso `BuscarOrcamentos` — filtro explícito enviado na requisição nunca é sobrescrito pela interpretação da IA, apenas complementado (mock de `AgenteInterpretadorConsultaGateway` e `AgenteEmbeddingGateway`); `tenantId` sempre presente e nunca sobrescrevível por parâmetro de busca. Coberto por `tests/bounded-contexts/busca-indexacao/application/buscar-orcamentos.test.ts`, exercitando `src/bounded-contexts/busca-indexacao/application/use-cases/buscar-orcamentos.ts` (implementação mínima do caso de uso, sem SDK AWS — infraestrutura de `BedrockInterpretadorConsultaGateway`/controller/IAM permanece T037/T039/T040).
