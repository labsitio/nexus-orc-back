...
**Amendment 2026-08-03 (ADR-008 de `specs/007-isolamento-multitenant-dados/plan.md`)**: os 2 Domain Events desta spec foram planejados sem `tenantId`; `OrcamentoValidadoEventACL` (T018) passa a também extrair `tenantId` do envelope v2 de 003, não apenas `itens`/`condicoesComerciais` (ADR-003 desta spec). T030 (#190) é gate explícito desse retrofit — ver `specs/007-isolamento-multitenant-dados/tasks.md` T041–T043.
...
**Confirmação (T006, issue #166)**: investigação do payload real (não apenas do `plan.md` da spec 003) confirmou que `OrcamentoValidado`/`OrcamentoValidadoComRessalva` ainda não traziam `itens`/`condicoesComerciais` — apenas `orcamentoId`/`ocorreuEm` (e `inconsistencias`, na ressalva). O enriquecimento decidido acima foi implementado na própria issue de coordenação (T006), diretamente no BC Validação (`orcamento-validado.event.ts`/`orcamento-validado-com-ressalva.event.ts`/`validar-orcamento.ts`), subindo `schemaVersion` para `2` (mesma convenção de amendment do ADR-005) — mudança aditiva, não quebra o consumidor já existente de Orquestração (`OrcamentoValidadoEventACL` daquele BC não lê os campos novos). `OrcamentoValidadoEventACL` desta spec (T018) já pode contar com `itens`/`condicoesComerciais` no payload a partir de agora. **`tenantId` não foi resolvido por este enriquecimento** — nenhum agregado do BC Validação (nem upstream, Ingestão/Extração) carrega `tenantId` hoje; permanece como a segunda dependência de coordenação, maior e cross-spec, já prevista no "Impactos futuros" do ADR-005 abaixo.
