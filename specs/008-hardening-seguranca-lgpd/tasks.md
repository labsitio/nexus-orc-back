...
- [ ] T008 Definir contrato dos Domain Events (`SolicitacaoEsquecimentoRegistrada`, `DadoPessoalAnonimizadoNoContexto`, `SolicitacaoEsquecimentoConcluida`, `SolicitacaoEsquecimentoPrazoExcedido`, `RetencaoAplicadaNoContexto`) como tipos TS em `src/platform/conformidade/domain/events/` — cada um com `schemaVersion: 1` e os campos definidos em `plan.md`. Depende de T004–T007.
- [ ] T009 Reaproveitar (import, não reimplementar) `EventPublisher`/`EventBridgePublisher` já estabelecido em 001 — confirmar publicação no mesmo bus `nexo-dominio-bus`, `detail-type` = nome do evento, `source = nexo.conformidade` para eventos publicados pelo componente de plataforma.
