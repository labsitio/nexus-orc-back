...
- [x] T019 [P] [US2] Teste de contrato: `GET /v1/auditoria/orcamentos/export` sem JWT válido retorna 401; com JWT de Tenant A nunca retorna evento de Tenant B, em `tests/bounded-contexts/acompanhamento/contract/exportacao-auditoria.test.ts`. Contrato de borda (Zod, `exportacao-auditoria.schema.ts`) fixado a partir de `docs/openapi.yaml` (`AuditoriaExportResponse`/`TrilhaAuditoriaEvento`) — query `periodo_inicio`/`periodo_fim`/`fornecedorId`/`status`/`cursor`/`limit` (1-200, default 50), item de resposta com `tenantId` (próprio tenant do requisitante, não é leak) e envelope `{ itens, proximoCursor }`; controller real (T029, bloqueado por T022-T028) ainda não existe, mesmo padrão de T024 da spec 004 (`indexacao-status.contract.test.ts`). Comportamento 401/cross-tenant fim-a-fim fixado via `problemDetailsSchema` (mesmo padrão do precedente) e exercido de fato pelo `TenantContextMiddleware` (já testado) e pelo controller real quando T029 for implementada. (#282, backend-reviewer: alinhamento ao openapi.yaml)
