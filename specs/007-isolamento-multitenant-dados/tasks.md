...
- [ ] T001 Criar `src/shared-kernel/tenant/tenant-id.vo.ts` — VO `TenantId`, UUID v7, validação de formato, sem lógica de negócio (ADR-004 do plan.md).
- [ ] T002 [P] Criar `src/shared-kernel/tenant/tenant-context.ts` — tipo `TenantContext` (request-scoped, nunca estado global mutável).
...
- [ ] T009 Checklist de infraestrutura (Terraform/CDK): garantir que nenhuma role IAM/DB de Lambda que acesse tabela tenant-scoped tenha `BYPASSRLS`. Aplicar a `ClassificadorLambdaRole`, `RevisorLambdaRole`, `ReceberOrcamentoLambdaRole`, `ConfirmarRevisaoHumanaLambdaRole`, `ConsultaStatusLambdaRole` (001) e às novas roles de Acompanhamento (T019).
...
- [ ] T017 [US1] Atualizar `ClassificarOrcamento`, `RevisarOrcamentoComIA`, `ConfirmarRevisaoHumana`, `ConsultarStatusOrcamento` para propagar/validar `tenantId` (comparação explícita contra o agregado antes de retornar dado; 404 em divergência) em `src/bounded-contexts/ingestao-identificacao/application/use-cases/`.
