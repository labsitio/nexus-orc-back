...
- [x] T017 [US1] Atualizar `ClassificarOrcamento`, `ConfirmarRevisaoHumana`, `ConsultarStatusOrcamento` para propagar/validar `tenantId` (comparação explícita contra o agregado antes de retornar dado; 404 em divergência) em `src/bounded-contexts/ingestao-identificacao/application/use-cases/`. (#639) Checkbox corrigido em T018 (#281, backend-reviewer NIT) — código já implementava desde o merge de #639.
- [x] T018 [US1] Migrar `DrizzleOrcamentoRepository` (001) para estender `DrizzleTenantScopedRepositoryBase` (T008), garantindo `SET LOCAL` em toda transação, em `src/bounded-contexts/ingestao-identificacao/infrastructure/persistence/drizzle-orcamento.repository.ts`. (#281) `TenantContext` é per-call, não singleton: os 4 casos de uso deste BC (`ReceberOrcamento`/`ClassificarOrcamento`/`ConfirmarRevisaoHumana`/`ConsultarStatusOrcamento`) agora recebem `CriarOrcamentoRepositorio` (fábrica `tenantId -> OrcamentoRepository`) em vez de uma instância fixa, e constroem o repositório dentro de `executar()` a partir do `tenantId` já recebido por parâmetro — nunca guardado como campo. Composition root (`src/composition/ingestao-identificacao.ts`) permanece com os 4 casos de uso como singletons; só a fábrica muda de forma.
...
- [x] T019 [P] [US2] Teste de contrato: `GET /v1/auditoria/orcamentos/export` sem JWT válido retorna 401; com JWT de Tenant A nunca retorna evento de Tenant B, em `tests/bounded-contexts/acompanhamento/contract/exportacao-auditoria.test.ts`. Contrato de borda (Zod, `exportacao-auditoria.schema.ts`) fixado a partir de `docs/openapi.yaml` (`AuditoriaExportResponse`/`TrilhaAuditoriaEvento`) — query `periodo_inicio`/`periodo_fim`/`fornecedorId`/`status`/`cursor`/`limit` (1-200, default 50), item de resposta com `tenantId` (próprio tenant do requisitante, não é leak) e envelope `{ itens, proximoCursor }`; controller real (T029, bloqueado por T022-T028) ainda não existe, mesmo padrão de T024 da spec 004 (`indexacao-status.contract.test.ts`). Comportamento 401 fixado via `problemDetailsSchema`; cross-tenant (endpoint de lista, sem 404 no openapi) fixado via `exportacaoAuditoriaResponseSchema` (200 com `itens` vazio) — ambos exercidos de fato pelo `TenantContextMiddleware` (já testado) e pelo controller real quando T029 for implementada. (#282, backend-reviewer: alinhamento ao openapi.yaml)
...
- [x] T041 Atualizar os 3 Domain Events de 003 (`src/bounded-contexts/validacao/domain/events/domain-event.ts` + `orcamento-validado.event.ts`, `orcamento-validado-com-ressalva.event.ts`, `orcamento-inconsistencia-detectada.event.ts`) para incluir `tenantId: string` **no mesmo bump** de `schemaVersion: 2` já exigido por ADR-003 de `specs/004-indexacao-busca-semantica-orcamentos/plan.md` (`itens`/`condicoesComerciais`, coordenação fechada em #166, código ainda não escrito) — um único PR, não dois. `tenantId` extraído do envelope v2 de 002 via `OrcamentoExtraidoEventACL` de 003. Depende de T040. **Concluído em duas partes**: `tenantId?: string` opcional adicionado ao envelope e aos 3 construtores (PR #637, commit 9ad8642) — wiring completo (ACL de entrada extrai e valida via `TenantId.de`, `OrcamentoValidacao.criar`/aggregate ganha `tenantId?` imutável, `ValidarOrcamento`/`RegistrarDecisaoHumanaValidacao` propagam até os 3 eventos de saída, coluna `tenant_id` nullable em `validacoes_orcamento`) entregue na issue #649. `schemaVersion` mantido conforme já estava (2 para `OrcamentoValidado`/`OrcamentoValidadoComRessalva`, 1 para `OrcamentoInconsistenciaDetectada` — ADR-003, assunto diferente de `tenantId`); nunca colapsado nem promovido (fora de escopo, é #632). tenantId ausente no envelope de entrada (002, ainda opcional) nunca é rejeitado — mesmo racional de T648/002: `OrcamentoValidacao` é sempre criado no caminho da fila, nunca há agregado pré-existente de outro tenant para divergir contra.
- [x] T042 Atualizar `OrcamentoValidadoEventACL` (spec 004 T018, `src/bounded-contexts/busca-indexacao/infrastructure/`) para extrair `tenantId` do envelope v2 de 003 (T041) e propagá-lo ao caso de uso `IndexarOrcamento` (já recebe `tenantId` como parâmetro dedicado, PR #574). Depende de T041. **Concluído (#584)**: `traduzir` valida `tenantId` via `TenantId.de` e o inclui em `OrcamentoValidadoEventACLResultado` (porta declara o campo obrigatório; validação/rejeição vive no adaptador de Infrastructure). Decisão vinculante: opção estrita — evento sem `tenantId` é rejeitado (`OrcamentoValidadoEventACLInvalidaError`), nunca indexado sem isolamento (base: zero tenant real em produção, #587/#297/T045). 003 ainda publica `tenantId` opcional (T041, expand/contract) — enquanto os sites de emissão de 003 não preencherem o campo, todo evento real será rejeitado por esta ACL; dependência registrada em comentário na issue #584, fora do escopo desta task (cutover de contrato é #632).
- [x] T043 Gate de desbloqueio: spec 004 T030/#190 (handler Lambda SQS `indexador-queue`) só pode ser implementada e mergeada depois de T042 mergeada — sem essa ordem, o handler não tem de onde extrair `tenantId` do evento `OrcamentoValidado` recebido. Nenhuma alteração de código nesta task, apenas remoção do bloqueio (T030 já está desenhada corretamente em `specs/004-.../tasks.md`). **Concluído (#585)**: cadeia serial confirmada fechada — #278 (T015/001), #582 (T040/002), #583 (T041/003), #584 (T042/004 ACL), todas `done`. Verificado em código: `OrcamentoValidadoEventACL` (`busca-indexacao/infrastructure/orcamento-validado-event.acl.ts`) extrai e valida `tenantId` do envelope via `TenantId.de`, rejeitando ausência (`OrcamentoValidadoEventACLInvalidaError`); os 3 eventos concretos de 003 (`orcamento-validado.event.ts`, `orcamento-validado-com-ressalva.event.ts`, `orcamento-inconsistencia-detectada.event.ts`) têm `tenantId: string` obrigatório desde o cutover (T046/#632, PR #655); `IndexarOrcamento` (004 T029) já recebe `tenantId` como parâmetro dedicado de `executar`, sem inferir do payload nem consultar outro BC. Board: `#190` já estava com label `ready` e sem `blocked` — nenhuma label a remover, gate já estava aberto.
- [x] T044 [P] Atualizar os Domain Events publicados por 005 (Orquestração, `src/bounded-contexts/orquestracao/domain/events/`) para incluir `tenantId`, extraído dos 3 eventos upstream (001/002/003, já v2 após T040/T041). Depende de T040 e T041. **Concluído parcialmente (#586)**: `tenantId?: string` opcional no envelope (`domain-event.ts`) e nos 5 construtores concretos (`DecisaoWorkflowEscalonadaParaComprador`, `IntegracaoExternaSolicitada`, `OrcamentoAprovadoParaProcessamento`, `OrcamentoEncaminhadoParaComprador`, `OrcamentoReenvioSolicitado`), inserido antes de `ocorreuEm` — mesmo padrão de T040/T041. `schemaVersion` mantido em 1 (expand/contract, ADR-008). Escopo entregue restrito aos 5 Domain Events publicados — a parte "contexto consolidado (ADR-001 de 005)" do enunciado original NÃO foi entregue aqui; rastreada separadamente em T044a (não toquei ACLs/agregado por instrução explícita de escopo).
- [x] T044a Propagar `tenantId` ao agregado `DecisaoWorkflow`/read-model "contexto consolidado" (ADR-001 de 005): schema `decisao-workflow.schema.ts` (coluna tenant), e as 3 ACLs de `orquestracao/` (`orcamento-classificado-event.acl.ts`, `orcamento-extraido-event.acl.ts`, `orcamento-validado-event.acl.ts`) extraindo `tenantId` dos eventos upstream (já opcional, T015/T040/T041). Depende de T044 (feito). **Concluído (#650, PR #653)**: 3 ACLs extraem/validam `tenantId` via `TenantId.de` (nunca rejeitam ausência); `DecisaoWorkflow` consolida `tenantId` entre os 3 upstreams com `TenantIdDivergenteError` (fail-fast) quando dois valores concretos divergem; propagado aos 5 eventos de saída via `ConsolidarEDecidirWorkflow`; coluna `tenant_id` (uuid, nullable) em `decisoes_workflow`, migração `drizzle/0019`. `RegistrarContextoExtracao` (consumidor de `OrcamentoExtraidoEventACL`) ainda não existe (#235/T029) — ACL pronta, sem consumidor de application ainda.
...
- [x] T047 Isolamento estrutural em 002/003/005: RLS + repositório tenant-scoped + `TenantContext` + `tenantId` obrigatório — fechar assimetria com 001/004 (issue #656). Depende de T046. **Concluído (#656, migração `drizzle/0020`)**: `ENABLE`/`FORCE ROW LEVEL SECURITY` + policy `tenant_isolation` + `tenant_id NOT NULL` (sem backfill, zero linha em produção) em `extracoes_orcamento`, `validacoes_orcamento`, `decisoes_workflow` e históricos; `DrizzleExtracaoOrcamentoRepository`/`DrizzleOrcamentoValidacaoRepository`/`DrizzleDecisaoWorkflowRepository` estendem `DrizzleTenantScopedRepositoryBase` (T008); os 4 controllers HTTP (`extracao/status`, `extracao/revisao-humana`, `validacao/status`, `validacao/decisao-humana`) extraem `TenantContext` e rejeitam cross-tenant (404, nunca 403); `ExtracaoOrcamento`/`OrcamentoValidacao`/`DecisaoWorkflow.tenantId` obrigatório desde a criação; guardas `*SemTenantIdError` removidas (estado que cobriam deixou de ser representável no tipo); testes de isolamento cross-tenant nos 4 endpoints; `tsc --noEmit`/`eslint` limpos; suíte completa verde com Postgres local up.

- [x] T046 Cutover de contract (ADR-008, decisão 4): tornar `tenantId` obrigatório e `schemaVersion: 2` nos envelopes de Domain Event de 001, 002, 003 e 005 — PR única e atômica, sem estado misto v1/v2. 004 fora de escopo (retrofit próprio já concluído, ADR-005/T013b). **Concluído (#632)**: os 15 eventos concretos dos 4 BCs (`schemaVersion = 2 as const`, `tenantId: string` obrigatório no construtor); os 4 `domain-event.ts` fecham `schemaVersion` em `2` (003 fecha a união `1 | 2` para `2`); todos os sites de emissão identificados sem `tenantId` (bugs de wiring pré-existentes em `receber-orcamento.ts`, `classificar-orcamento.ts`, `confirmar-revisao-humana.ts` de 001) corrigidos; as ACLs cross-BC (`validacao/infrastructure/orcamento-extraido-event.acl.ts`, as 3 de `orquestracao/infrastructure/orcamento-*-event.acl.ts`) passam a rejeitar evento sem `tenantId` em runtime (mesma decisão vinculante de `busca-indexacao/infrastructure/orcamento-validado-event.acl.ts`/T042); `tsc --noEmit`/`eslint` limpos sem `any`/`as`/`@ts-ignore`; suíte completa verde (176 arquivos, 1065 testes, zero skip com Postgres local up) sem `expected fail` remanescente de tenant. Aggregate (`Orcamento`/`ExtracaoOrcamento`/`OrcamentoValidacao`) e coluna `tenant_id` de 002/003/005 permanecem opcionais/nullable — fora do escopo desta issue (legado/consolidação incremental, não contrato de evento publicado); guardas de fail-fast (`ExtracaoSemTenantIdError`, `OrcamentoValidacaoSemTenantIdError`, `DecisaoWorkflowSemTenantIdError`) protegem os poucos pontos de emissão sem `TenantContext` wireado (endpoints de decisão/confirmação humana de 002/003) sem inventar o campo.

- [x] T048 [ADR-010 T6] Teste adversarial de papel forjado em body/header/query nas 2 rotas gated por `criarExigenciaPapel` — `POST /v1/orcamentos/{orcamentoId}/workflow/decisao-humana` (`comprador-responsavel`) e `POST /v1/configuracoes/faixas-preco-categoria` (`compliance-admin`), mirroring #635 (mesma garantia para `tenantId`). **Concluído (#690)**: `tests/security/verificacao-papel/papel-forjado-http-adversarial.test.ts` — `criarTenantContextMiddleware` real (só `aws-jwt-verify` mockado, nenhum fake de auth que aceite papel injetado), 9 casos: papel forjado em body/header/query sem o grupo real → 403 (corpo/publisher intactos), e contraprova com o grupo real no token → 200/201. Verificado manualmente que a suíte fica vermelha ao reintroduzir leitura de `request.body.papeis` em `extrairPapeis`/`TenantContextMiddleware` (revertido antes do commit).
