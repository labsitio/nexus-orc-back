...
- [ ] T044a Propagar `tenantId` ao agregado `DecisaoWorkflow`/read-model "contexto consolidado" (ADR-001 de 005): schema `decisao-workflow.schema.ts` (coluna tenant), e as 3 ACLs de `orquestracao/` (`orcamento-classificado-event.acl.ts`, `orcamento-extraido-event.acl.ts`, `orcamento-validado-event.acl.ts`) extraindo `tenantId` dos eventos upstream (já opcional, T015/T040/T041). Depende de T044 (feito). Follow-up identificado na revisão de #586 — sem issue aberta ainda.
