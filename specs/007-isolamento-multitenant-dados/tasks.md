...
- [x] T044 [P] Atualizar os Domain Events publicados por 005 (Orquestração, `src/bounded-contexts/orquestracao/domain/events/`) e o contexto consolidado (ADR-001 de 005) para incluir `tenantId`, extraído dos 3 eventos upstream (001/002/003, já v2 após T040/T041). Depende de T040 e T041. **Concluído (#586)**: `tenantId?: string` opcional no envelope (`domain-event.ts`) e nos 5 construtores concretos (`DecisaoWorkflowEscalonadaParaComprador`, `IntegracaoExternaSolicitada`, `OrcamentoAprovadoParaProcessamento`, `OrcamentoEncaminhadoParaComprador`, `OrcamentoReenvioSolicitado`), inserido antes de `ocorreuEm` — mesmo padrão de T040/T041. `schemaVersion` mantido em 1 (expand/contract, ADR-008). "Contexto consolidado" (ADR-001 de 005) é o agregado `DecisaoWorkflow`/VOs de contexto (classificação/extração/validação), populado pelas ACLs — fora de escopo aqui (wiring, ACLs não tocadas enquanto o campo é opcional).
