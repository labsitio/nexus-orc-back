...
// 003 T014) e a busca híbrida (filtro determinístico + distância vetorial).
//
...
describe.skipIf(!DATABASE_URL)('DrizzlePgvectorIndiceOrcamentoRepository (Postgres real)', () => {
  ...
  beforeAll(async () => {
    ...
    db = drizzle(client);
    repo = new DrizzlePgvectorIndiceOrcamentoRepository(db);
  });
  ...
  it('duas chamadas concorrentes de upsert() para o mesmo orcamentoId (retry) produzem exatamente 1 entrada de histórico', async () => {
    ...
    const repoB = new DrizzlePgvectorIndiceOrcamentoRepository(drizzle(clienteB));
