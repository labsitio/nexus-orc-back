// Integration test: exercita a migração baseline do BC Validação (T002)
...
// provar aqui é que o schema `validacao` e as duas tabelas existem. Colunas
// reais (dados_extraidos/inconsistencias JSONB, histórico append-only)
// chegam em T013, com seu próprio teste.
//
...
describe.skipIf(!DATABASE_URL)('schema validacao.validacoes_orcamento* (Postgres real)', () => {
  ...
  it('migração cria o schema validacao e as tabelas baseline', async () => {
    const schemas = await client.query<{ schema_name: string }>(
      `select schema_name from information_schema.schemata where schema_name = 'validacao'`,
    );
    expect(schemas.rows).toHaveLength(1);

    const tabelas = await client.query<{ table_name: string }>(
      `select table_name from information_schema.tables
       where table_schema = 'validacao' and table_name in ('validacoes_orcamento', 'validacoes_orcamento_historico')`,
    );
