...
import { criarContextoExtracaoQueueHandler } from '../../../../src/bounded-contexts/orquestracao/interface/events/contexto-extracao-queue.handler.js';
import { OrcamentoExtraidoEventACL } from '../../../../src/bounded-contexts/orquestracao/infrastructure/orcamento-extraido-event.acl.js';
import type { RegistrarContextoExtracao } from '../../../../src/bounded-contexts/orquestracao/application/use-cases/registrar-contexto-extracao.js';
...
): RegistrarContextoExtracao {
  return { executar } as unknown as RegistrarContextoExtracao;
}
...
function payloadValido(orcamentoId: string, detailType: string, tenantId?: string | null) {
  return {
    ...
    itens: [
      {
        descricao: { valor: { descricao: 'Notebook' } },
        quantidade: { valor: 1 },
        precoUnitario: { valor: { valorCentavos: 100000, moeda: 'BRL' } },
      },
    ],
    condicoesComerciais: {
      condicoesPagamento: { valor: '30 dias' },
      prazoValidade: { valor: '15 dias' },
      condicoesEntrega: { valor: 'FOB' },
    },
...
function envelopeEventBridge(
  detailType: 'OrcamentoExtraido' | 'OrcamentoExtraidoComPendenciaConfirmada',
  orcamentoId: string,
...
describe('criarContextoExtracaoQueueHandler', () => {
  it('invoca RegistrarContextoExtracao.executar com o detail bruto para cada mensagem, aceitando os 2 detail-types', async () => {
    const executar = vi.fn().mockResolvedValue(undefined);
    const handler = criarContextoExtracaoQueueHandler(
      useCaseFake(executar),
      new OrcamentoExtraidoEventACL(),
    );

    const resposta = await handler({
      Records: [
        { messageId: 'm1', body: envelopeEventBridge('OrcamentoExtraido', ORCAMENTO_ID_1) },
        {
          messageId: 'm2',
          body: envelopeEventBridge('OrcamentoExtraidoComPendenciaConfirmada', ORCAMENTO_ID_2),
        },
      ],
    });
  ...
  it('reporta só o item falho (batch item failure) sem interromper o processamento das demais mensagens', async () => {
    ...
    const resposta = await handler({
      Records: [
        { messageId: 'm1', body: envelopeEventBridge('OrcamentoExtraido', ORCAMENTO_ID_FALHA) },
        { messageId: 'm2', body: envelopeEventBridge('OrcamentoExtraido', ORCAMENTO_ID_OK) },
      ],
    });
  ...
  it('(ADR-008/#632) rejeita via ACL (batch item failure) quando tenantId está ausente no envelope', async () => {
    ...
    const resposta = await handler({
      Records: [
        { messageId: 'm1', body: envelopeEventBridge('OrcamentoExtraido', ORCAMENTO_ID_1, null) },
      ],
    });
